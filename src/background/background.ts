...
chrome.runtime.onMessage.addListener((msg: KspMessage, sender, sendResponse) => {
  ...
  (async () => {
    ...
    if (msg.type === 'KSPULSE_GET_STATE') {
      ...
      if (!state) { sendResponse(null); return; }
      sendResponse({
        ...state,
  ...
  switch (msg.type) {
    case 'KSPULSE_CONSOLE': {
      ...
      await tabStateManager.addConsoleEntry(tabId, {
        level: m.level, category: m.category,
    ...
    case 'KSPULSE_NAV': {
      ...
      await tabStateManager.updateNav(tabId, {
        ttfb: m.ttfb, domContentLoaded: m.domContentLoaded,
        loadComplete: m.loadComplete, protocol: m.protocol, transferSize: m.transferSize,
      });
      break;
    }
    ...
    case 'KSPULSE_LONG_TASK': {
      ...
      await tabStateManager.addLongTask(tabId, {
        duration: m.duration, blockingDuration: m.blockingDuration,
        startTime: m.startTime, entryType: m.entryType,
      });
      break;
    }
  }
...
async function refreshBadge(tabId: number): Promise<void> {
  const state = await tabStateManager.get(tabId);
  if (!state) return;
  const health = state.health === 'loading' ? 'loading' : classifier.overallHealth(state);
