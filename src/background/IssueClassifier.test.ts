...
describe('IssueClassifier', () => {
  it('returns no issues for clean state', () => {
    const state = makeState({
      requests: [{ requestId: '1', url: 'https://example.com/', type: 'main_frame', timeStamp: 0, status: 'complete', statusCode: 200 }],
    });
  ...
  it('returns TOTAL_FAILURE when all requests fail', () => {
    const state = makeState({
      requests: [
        { requestId: '1', url: 'https://example.com/', type: 'main_frame', timeStamp: 0, status: 'failed', error: 'ERR_CONNECTION_TIMED_OUT' },
        { requestId: '2', url: 'https://example.com/app.js', type: 'script', timeStamp: 0, status: 'failed', error: 'ERR_CONNECTION_TIMED_OUT' },
        { requestId: '3', url: 'https://example.com/style.css', type: 'stylesheet', timeStamp: 0, status: 'failed', error: 'ERR_CONNECTION_TIMED_OUT' },
      ],
    });
  ...
  it('returns SERVER_ERROR for 5xx responses', () => {
    const state = makeState({
      requests: [
        { requestId: '1', url: 'https://example.com/', type: 'main_frame', timeStamp: 0, status: 'complete', statusCode: 200 },
        { requestId: '2', url: 'https://example.com/api', type: 'xmlhttprequest', timeStamp: 0, status: 'complete', statusCode: 503 },
        { requestId: '3', url: 'https://example.com/app.js', type: 'script', timeStamp: 0, status: 'complete', statusCode: 504 },
      ],
    });
    const issues = classifier.classify(state);
  ...
  it('returns SLOW_TTFB for nav timing above poor threshold', () => {
    const state = makeState({
      requests: [{ requestId: '1', url: 'https://example.com/', type: 'main_frame', timeStamp: 0, status: 'complete', statusCode: 200 }],
      nav: { ttfb: 2500, domContentLoaded: 3000, loadComplete: 3500, protocol: 'h2', transferSize: 12000 },
    });
    const issues = classifier.classify(state);
  ...
  it('score is 0 for critical, ~60 for high, ~80 for medium, 100 for clean', () => {
    const critical = makeState({ requests: Array.from({ length: 3 }, (_, i) => ({ requestId: `${i}`, url: 'x', type: 'script', timeStamp: 0, status: 'failed' as const, error: 'ERR_FAILED' })) });
    expect(classifier.healthScore(critical)).toBe(0);

    const oneHigh = makeState({
      requests: [
        { requestId: '1', url: 'https://x.com/', type: 'main_frame', timeStamp: 0, status: 'complete', statusCode: 200 },
        { requestId: '2', url: 'https://x.com/app.js', type: 'script', timeStamp: 0, status: 'complete', statusCode: 500 },
      ],
    });
    expect(classifier.healthScore(oneHigh)).toBe(60);

    const oneMedium = makeState({
      requests: [{ requestId: '1', url: 'https://x.com/', type: 'main_frame', timeStamp: 0, status: 'complete', statusCode: 200 }],
      console: [{ level: 'error', category: 'REACT_ERROR', message: 'Hydration failed', timestamp: 0 }],
    });
