...
export function MetricRow({ summary }: Props) {
  ...
  const loadLabel = loadMs != null
    ? loadMs >= 1000 ? `${(loadMs / 1000).toFixed(1)}s` : `${loadMs}ms`
    : '—';
  ...
  return (
    <div style={{
      ...
      <Dot />
      <span title="Total requests" style={{ color: failedReqs > 0 ? 'var(--health-warning)' : undefined }}>
        {reqLabel} req
      </span>
      <Dot />
      <span title="Protocol" style={{ color: secure ? 'var(--health-good)' : 'var(--health-warning)' }}>
...
function Dot() {
  return <span aria-hidden style={{ opacity: 0.3 }}>·</span>;
}
